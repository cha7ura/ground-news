...
// Switches between local (LM Studio/Ollama) and cloud (OpenAI/OpenRouter) based on APP_ENV
//
...
// Set EMBEDDING_PROVIDER in your env to switch between them.
...
function getLLMConfig(appEnv: AppEnv): LLMConfig {
  if (appEnv === 'local') {
    return {
      provider: 'lmstudio',
      baseUrl: process.env.LMSTUDIO_BASE_URL || 'http://localhost:1234/v1',
      model: process.env.LMSTUDIO_LLM_MODEL || 'local-model',
      apiKey: process.env.LMSTUDIO_API_KEY || 'lm-studio',
    };
  }
