...
export function BiasIndicator({ 
  ...
  if (total === 0) {
    return (
      <div className={cn('w-full', className)}>
        <div className={cn('rounded-full bg-gray-200 overflow-hidden', heights[size])}>
          <div className="h-full w-full bg-gray-300" />
        </div>
        {showLabels && (
          <p className="text-xs text-gray-400 mt-1 text-center">No coverage data</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className={cn('w-full', className)}>
      {/* Bias bar */}
      <div className={cn('rounded-full bg-gray-200 overflow-hidden flex', heights[size])}>
        {percentages.left > 0 && (
          <div 
      ...
      {showLabels && (
        <div className="flex justify-between text-xs mt-1.5">
          ...
          <span className={cn(
            'flex items-center gap-1',
            percentages.center > 0 ? 'text-bias-center' : 'text-gray-400'
          )}>
