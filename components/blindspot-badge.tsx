  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <div className="flex justify-between text-xs">
        <span className={cn(
          leftPercentage > 0 ? 'text-bias-left font-medium' : 'text-gray-400'
        )}>
          {leftPercentage > 0 ? `Left ${leftPercentage}%` : 'L 0%'}
        </span>
        <span className={cn(
          centerPercentage > 0 ? 'text-bias-center font-medium' : 'text-gray-400'
        )}>
          Center {centerPercentage}%
        </span>
        <span className={cn(
          rightPercentage > 0 ? 'text-bias-right font-medium' : 'text-gray-400'
        )}>
