  ...
  if (stories.length === 0) {
    return (
      <Card className="p-12 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-600 mb-2">No briefing available</h3>
    ...
    <div className="space-y-6">
          ...
          <Card className="overflow-hidden hover:shadow-lg transition-all duration-200 group">
            <div className="flex flex-col md:flex-row">
              ...
              <div className={cn(
                ...
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 group-hover:text-brand-primary transition-colors">
                  {featured.title}
                </h2>
                {featured.summary && (
      ...
      {rest.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-lg font-bold text-gray-900">More Stories</h2>
          {rest.map((story, index) => (
            <Link key={story.id} href={`/story/${story.id}`}>
              <Card className="p-4 hover:shadow-md transition-all duration-200 hover:border-brand-primary/30 group">
                <div className="flex gap-4">
                  {/* Index number */}
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-sm font-bold text-gray-500 group-hover:bg-brand-primary group-hover:text-white transition-colors">
                    {index + 2}
                  </div>
                  ...
                  <ChevronRight className="h-5 w-5 text-gray-400 flex-shrink-0 self-center opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </Card>
            </Link>
          ))}
...
function BriefingSkeleton() {
  return (
    <div className="space-y-6">
      <Card className="p-4 bg-gray-200 animate-pulse h-16" />
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Card className="p-4 mt-8 flex items-center justify-between">
        <div>
          <h3 className="font-medium text-gray-900">Previous Briefings</h3>
          <p className="text-sm text-gray-500">Browse past daily briefings</p>
        </div>
        <Link 
