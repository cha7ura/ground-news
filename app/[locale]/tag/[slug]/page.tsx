    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {stories.length > 0 && (
            <section>
              <h2 className="text-xl font-bold text-gray-900 mb-4">{dict.tags.related_stories}</h2>
        ...
        <div className="space-y-6">
          ...
          {relatedTags.length > 0 && (
            <Card className="p-4">
              <h3 className="font-semibold text-gray-900 mb-3">{dict.tags.related_tags}</h3>
              <div className="flex flex-wrap gap-2">
                {relatedTags.map((relTag) => (
                  <Link
                    key={relTag.id}
                    href={`/${locale}/tag/${relTag.slug}`}
                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-100 hover:bg-brand-primary/10 hover:text-brand-primary text-gray-700 rounded-full text-sm transition-colors"
                  >
                    <span>{getLocalizedTagName(relTag, locale)}</span>
                    <span className="text-xs text-gray-400">({relTag.article_count})</span>
                  </Link>
                ))}
              </div>
            </Card>
